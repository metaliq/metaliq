...
import { FieldKey, Meta, $Fn } from "../../../meta"
 ...
 */
export const sameAs = <T, P>(other: FieldKey<P>, msg?: string): Validator<T, P> => ({ value, parent }) => {
  const otherMeta = parent[other] as Meta<any>
  return value === otherMeta.$.value || msg || `Does not match ${otherMeta.$.spec.label}`
}

export const transform = <T> (fn: $Fn<T, any, T>): Validator<T> => $ => {
  $.meta.$.value = fn($)
  return true
}
...
export const siblingsBlankOr = <T, P>(siblings: Array<FieldKey<P>>, other: Validator<T>): Validator<T, P> => $ =>
  !siblings.map(key => $.parent[key].$.value).filter(Boolean).length || other($)

...
describe("Hide / Show rules", () => {
  it("should hide or show a field depending on a rule", async () => {
    ...
    mApplication.principal.isSelfEmployed.$.term("hidden").should.be.a("boolean").equal(true)

    await up(contact => {
      contact.age = 21
    }, appVal.principal)()
    mApplication.principal.isSelfEmployed.$.term("hidden").should.be.a("boolean").equal(false)
  })
})
